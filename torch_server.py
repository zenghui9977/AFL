...
# 利用pytorch对模型参数就进行赋值、加减等处理
def set_global_model_zero(model_weight):
    for key in model_weight.keys():
        model_weight[key] += - model_weight[key]
    return model_weight


def accumulate_global_model_weight(model_weight, local_weight, local_size):
    for key in model_weight.keys():
        model_weight[key] += local_weight[key] * local_size
    return model_weight


def ave_global_model_weight(model_weight, data_size):
    for key in model_weight.keys():
        model_weight[key] = model_weight[key] / data_size
    return model_weight

def global_weight_nan(model_weight):
    for k, v in model_weight.items():
        temp = np.isnan(v.numpy()).reshape(-1, 1).flatten()
        if True in temp:
            return True
    return False
...
for sim in sim_runs:
    ...
    for case in case_range:
        ...
        for tau_setup in tau_setup_all:
            ...
            w_gloabl_init = model.state_dict()
            w_global = w_gloabl_init
            ...
            while True:
                ...
                # 初始化为0的全局参数
                w_global = set_global_model_zero(w_global)
                ...
                for n in range(0, n_nodes):
                    ...
                    w_global = accumulate_global_model_weight(w_global, w_local, data_size_local)
                ...
                w_global = ave_global_model_weight(w_global, data_size_total)

                # if True in np.isnan(w_global):
                if global_weight_nan(w_global):
